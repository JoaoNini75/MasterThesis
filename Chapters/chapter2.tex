...
%---INTRODUZIR centrado \emph{S} ≜ if grade >= 10 then reward := 10 else reward := 0 
Hoare logic can be used to prove this triple:
%---INTRODUZIR centrado {grade >= 0 ∧ grade <= 20} \emph{S} {(grade >= 10 ∧ reward = 10) ∨ (grade < 10 ∧ reward = 0)}
...
It is usually seen as an extension of Caml (dialect of the Meta Language) that includes object-oriented features, making OCaml a very versatile language.
