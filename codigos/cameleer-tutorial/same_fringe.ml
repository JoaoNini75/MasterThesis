...
module Make (Eq : EQUAL) = struct
  ...
  (*@ function elements (t: tree) : Eq.elt list =
        match t with
        | Empty -> []
        | Node l x r -> elements l @ (x :: elements r) *)

  type zipper = (Eq.elt * tree) list

  (*@ function enum_elements (e : zipper) : Eq.elt list =
        match e with
        | [] -> []
        | (x, r) :: e -> x :: (elements r @ enum_elements e) *)

  let rec mk_zipper (t : tree) (e : zipper) =
    match t with
    | Empty -> e
  ...
  let rec eq_zipper (e1 : zipper) (e2 : zipper) =
    match (e1, e2) with
    | [], [] -> true
    | (x1, r1) :: e1, (x2, r2) :: e2 ->
        Eq.eq x1 x2 && eq_zipper (mk_zipper r1 e1) (mk_zipper r2 e2)
    | _ -> false
  (*@ b = eq_num e1 e2
  ...
  let same_fringe (t1 : tree) (t2 : tree) =
    eq_zipper (mk_zipper t1 []) (mk_zipper t2 [])
  (*@ b = same_fringe t1 t2
