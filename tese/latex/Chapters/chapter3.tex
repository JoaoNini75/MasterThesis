...
For example, a program \emph{$P_1$} that consists in two assignments and a while loop is not equivalent to another program \emph{$P_2$} if the latter's structure is a single for loop.
...
Note that this definition is useful when reading about self-composition and cross-products below in this chapter.
...
Non-interference separates the state of a program into a public and a secret part and, by observing its execution, determines if there was an information leak on the secret part of the state.
...
One of the main features of self-composition is its expressiveness, and there is no need to prove the soundness of type systems, since it is based on logic.


\subsection{Exploring non-interference}
\label{subsec:exploring_non_interference}

In order to give an example of self-composition in the next subsection, we now present a simple program that respects the properties of termination-insensitivity and non-interference.
...
Also, to refer to the disjoint union of two memories, we will use the $\uplus$ symbol.
...
We are able to characterize information flow policies, which include some types of declassification, through the replacement of the =- relation by another (partial) equivalence relation.
...
With this, instead of resorting to program analysis and proof rules developed specifically for translation validation, it became possible to use existing methods and tools to prove properties of a single program. 
...
The approach was to divide the analysis in two phases, a faster one first and a more precise one after that.
...
The results presented in the cited work reveal that CoVaC was able to verify a very considerable set of optimizations of LLVM~\cite{llvm}, a complex modern compiler.
...
Consider the following example, composed of a source program and its optimized version:
...
We mark $read$ and $write$ as observable transitions, while assignments are internal but still visible to the cross-product as assignment labels.
...
For a program $C$, we say that an assertion network is $invariant$ if, for every state $<n;\overrightarrow{d}>$ occurring in a computation, $d \vDash \varphi_n$.
...
Intuitively, this approach proves correctness for the following three reasons.
...
Together, these satisfy the cross-product's definition of a witness, therefore, the translation is correct.
...
Therefore, as a way of getting around that limitation, we can take advantage of the existing tool support if we are able to soundly reduce relational verification tasks into standard verification ones.
...
One of the two main reasons is that the existing automatic safety analysis tools available are not powerful enough to verify most of the realistic problems and, when they can, there is a lack of performance.
...
These suffer from the constraint of structural equivalence, making it impossible to reason about properties or program optimizations that are based on different control flows.

Product Programs~\cite{DBLP:conf/fm/BartheCK11} appear to be the best path to follow, since they represent a general notion that combines the flexibility of self-composition in executing asynchronous steps and the efficiency of cross-products when it comes to treating synchronous steps. 
...
As a way to provide a first intuition of the construction of a product from structurally dissimilar programs before we dive into the definitions and rules later in this subsection, consider the simple \hyperref[fig:pp_first_intuition]{example} below (assume 0 $\leq$ N).
...
Rather than relying purely on self-composition, this idea maximizes synchronization and differs from a functional interpretation of the product components.
Furthermore, this approach also considerably reduces the complexity of the invariants required to prove the product program.
...
On the other hand, if we construct the product program, the loop invariant \emph{i = j $\land$ x = y} is enough to verify that the first two programs above satisfy the pre and post-relation \emph{x = y}.
...
\bigskip
Since our objective is to reduce the validity of Hoare quadruples to the validity of Hoare triples, we must say that we establish our notion of a valid Hoare triple as stronger than usual.
...
\bigskip
The \hyperref[fig:product_construction_reduction]{figure} describes the set of rules that define judgments of the form $\vdash$ \emph{c} $\succcurlyeq$ \emph{c'}.
...
Loop alignment is an optimization that consists of improving cache effectiveness by increasing the proximity of the memory locations accessed in each iteration of the loop.
\begin{figure}
  \centering
...
In the example, \emph{j} is a derived induction variable defined as a linear function of the induction variable \emph{i}.
...
This tool appears in a context where two types of tools exist, but the differences between them are quite relevant.
One of the types of tools is the ones that do not support well the relational verification of programs that dynamically allocate mutable state; these take much weight off users' shoulders but are only useful to a restrict range of programs.
...
It does not adapt well to complex programs and relational properties; it also prevents us from exploiting similarities between the programs involved or reasoning through relational specs in a modular way.
The second technique uses appropriate program alignments to prove the desired relational property.
WhyRel uses biprograms to represent those alignments, which point out similarities between the two programs so that we can reason about their effects jointly.
...
Since this is challenging on a technical level, WhyRel specifies encapsulation as a \emph{dynamic boundary}, a kind of dynamic frame.
...
After that, WhyRel translates these inputs into WhyML code, encoding them in a way that accurately captures the representation of the heap model and fine-grained framing formalized in relational region logic.
...
Additionally, to prove this invariant, we need to state that the inner loop of the left side has the same effect as the single assignment on the right: incrementing the \emph{result} by \emph{m}.
In this case, that reasoning comes to life through the assertion after the left inner loop and before the assignment to \emph{i}.
...
\emph{ex1} and \emph{ex2} structures are very similar and the example is simplified by setting the \emph{z} to 4! and $2^4$, respectively.
...
For the other situations, when a side will not update those variables in the current iteration, the alignment of the iteration on the other side should be done by executing nothing, represented by the \emph{skip} command.
...
The only assignment in this case is the increment of the \emph{w} variable, since the alignment guard is contradictory with the $if$'s condition.
...
The second phase consists of the introduction of annotations in the biprogram, such as assertions or loop invariants, by WhyRel, with the objective of ensuring adequacy.
In a hypothetical case where a biprogram aligns two loops in lockstep, this tool would add, as a relational loop invariant, the equivalence of the two loop guards.
...
In a similar way, $\mu$ translates unary programs into WhyML programs, $\epsilon$ maps expressions into WhyML expressions, and $F$ transforms a restricted set of relation formulas also into WhyML expressions.
...
$\lfloor C \rfloor$ is simply syntactic sugar for ($C|C$) that are translated by rule 3, with the exception of method calls.
Aligning method calls that establish that the relational spec associated with the method allows procedure-modular reasoning about relational properties.
...
Rule 4 represents that a sequence of commands in the biprogram is translated to a sequence of commands in the product program as well.
...
Regarding conditionally aligned loops, described by rule 8, the pattern shown by the alignment guards $P | P'$ is captured by the generated loop body.
This means that if the left (resp. right) guard evaluates to true and $P$ (resp. $P'$) holds, a left-only (resp. right-only) iteration is executed; if not, a lockstep iteration is performed.
...
WhyRel supports several types of elements to be in the alignment guards: the commonly used boolean connectives, one-sided boolean expressions, one-sided points-to-assertions, and, as said several times before, agreement formulas.
...
The best solution we found is the use of labels to capture the state of the program before the execution of the inner while loop, therefore allowing us to access the value of $result$ in the previous iteration.
