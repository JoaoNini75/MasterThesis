...
\iffalse\begin{figure}[ht]
  ...
  \begin{lstlisting}
BJC|C’K(Γl,Γr) =̂ UJCK(Γl); UJC’K(Γr)

BJ⌊m(x|y)⌋K(Γl,Γr) =̂ Φ(m)(Γl.st,Γr.st, EJxK(Γl), EJyK(Γr))

BJ⌊C⌋K(Γl,Γr) =̂ BJC|CK(Γl,Γr)

BJCC; DDK(Γl,Γr) =̂ BJCCK(Γl,Γr); BJDDK(Γl,Γr)

BJvar x:T|x:T’ in CCK(Γl,Γr) =̂ let xl = def(T) in let xr = def(T’) in
BJCCK([Γl | x : xl], [Γr | x : xr])

BJif E|E’ then CC else DDK(Γl,Γr) =̂ assert {EJEK(Γl) = EJE’K(Γr)};
if EJEK(Γl) then BJCCK(Γl,Γr)
else BJDDK(Γl,Γr)

BJwhile E|E’ do DDK(Γl,Γr) =̂ while EJEK(Γl) do
invariant {EJEK(Γl) = EJE’K(Γr)}
BJCCK(Γl,Γr)

BJwhile E|E’. P|P do DDK(Γl,Γr) =̂

while (EJEK(Γl) ∨ EJE’K(Γr)) do invariant {A}
if (EJEK(Γl) ∧ FJPK(Γl,Γr)) then UJ↼−CCK(Γl)

else if (EJE’K(Γr) ∧ FJP ′K(Γl,Γr)) then UJ−⇀CCK(Γr) else BJCCK(Γl,Γr)

where A ≡ (EJEK(Γl) ∧ FJPK(Γl,Γr)) ∨ (EJE′K(Γr) ∧ FJP ′K(Γl,Γr)) ∨
(¬EJEK(Γl) ∧ ¬EJE′K(Γr)) ∨ (EJEK(Γl) ∧ EJE′K(Γr))
  \end{lstlisting}
  \end{minipage}
