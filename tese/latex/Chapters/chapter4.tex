...
WhyRel describes a practical approach to the verification of program equivalence based on product programs.
...
Its authors deeply explore the problems that pointer based programs bring to the table, but this is not an objective of this thesis.
...
We did that in order to allow programmers who are used to that language to verify the equivalence of their programs without needing to learn a new language or completely rewrite their code or specification.
...
All of the work done on this thesis is available at \url{https://github.com/JoaoNini75/MasterThesis}, which includes bip2ml's code, this thesis's LaTeX code, and other related material.
...
The first component is the lexer, which defines what the keywords and tokens accepted by BipLang are; if any error occurs during this phase, bip2ml outputs \emph{"lexical error"} to the terminal.
We used ocamllex~\cite{ocamllex} to develop our lexer.
Representing the second part of the transpiler, we have the parser, which takes the lexer's output and, by comparing it with the rules we define that are valid BipLang constructions, determines if the source code's structure is valid or not.
...
Finally, we named the last part of our transpiler the printer, since its function is to receive the OCaml AST from the translator and output to the terminal or to a file the final OCaml code with all the GOSPEL specifications.
...
Although it is dispensable, it certainly was extremely helpful during the whole development and testing of bip2ml, and it may also be a relevant aid to anyone who programs in BipLang.
...
They both refer to identifiers, but $id$ refers to an all lowercase identifier, while $idc$ stands for "capitalized id", so its first letter is capitalized.
...
If you do not want to write the specification at a given moment but want bip2ml to execute without a parsing error, you can simply do the following:
\begin{lstlisting}[mathescape, basicstyle=\ttfamily, columns=flexible,
                    emph={type, and, let, rec, if, then, else, mod, in, for, while, do, done, to, begin, end, assert, match, with, of, open, include,ref},
                    emphstyle=\ttfamily\bfseries\color{myorange}]
...
Moving on to the \hyperref[fig:trans-ex-second]{second} example, pay attention to the use of floors in the parameters and on the type returned by the $second\_bip$ function.
...
This one showcases the use of rule 8, which translates conditionally aligned loops.
This rule takes as input a while loop with two alignment guards and outputs a while loop with a transformed body and a rather complex invariant, $A$.

$A$ corresponds to the logical disjunction of 4 main formulas, which are all conjunctions themselves.
