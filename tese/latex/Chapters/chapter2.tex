...
If we keep the programs \emph{C1;C2} and \emph{C1';C2'} \emph{separable} by renaming their variables and if \emph{$\Phi$ = $\Phi$'} and \emph{$\Psi$ = $\Psi$'}, we get to the self-composition rule in the \hyperref[fig:extended_rules_rhl]{figure}.
...
Alternatively, if no errors are found, a series of verification conditions (VCs) are the output of the respective VCGEN.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[max width=\textwidth]{gcd_why3}
  \caption{Verifying \textbf{gcd.ml} with the Why3 IDE. TODOOO: FIX IMAGE} 
  \label{fig:gcd_why3}
\end{figure}
