...
\subsubsection{Evolution from Classical\protect\footnote{\ When we mention Classical Hoare Logic, we mean the original Hoare logic developed by Tony Hoare.} Hoare Logic}
...
One common example is compiler optimizations, since they aim to make the code more performant but still output the same results.
Another example is represented by a "client" program that uses an abstract data type and does not care about which implementation the "server" provides.
...
Finally, consider two equal programs \emph{$P_1$} and \emph{$P_2$} again.
...
Consider the following grammar, which will be used in the rules in this subsubsection.
...
The ellipses above mean that other forms of integer expressions could be added according to need.
...
To allow this and other important derivations, we present now 4 rules that enable the separate reasoning of \emph{$C_1$} and \emph{$C_2$}.
...
This problem can be overcome when we extend the base set with one more rule: self-composition.
...
There is, however, strong evidence that OCaml is far from being an "academia only" programming language.
OCaml is used in tech giants such as Meta and Microsoft, in Bloomberg L.P. (which even created an OCaml to JavaScript compiler backend), in popular tools like Docker, and in many other companies~\cite{ocaml_companies}.
But there is more.
...
Why3 is a platform whose objective is the verification of programs in a deductive way, presenting itself as a \emph{"a front-end to third-party theorem provers"}~\cite{why3}.
Those theorem provers can be put to work together and vary greatly in nature, ranging from SMT solvers (for example, Z3 or cvc5) to TPTP provers and even interactive proof assistants (such as Coq or Isabelle).
...
As a way of demonstrating how Why3 works in practice, here is a preview of what is possible to do with product programs, an important concept that we will discuss in detail in the next chapter.
...
GOSPEL is based in Separation Logic and significantly improves the experience of the people who write or read the specifications, by making them much more concise.
...
This serves as a guarantee that the recursion does not go on forever.
...
The third step consists of Cameleer itself translating the complete AST into a description in WhyML that is equivalent; Why3 then takes that as input.
...
Alternatively, if no errors are found, a series of verification conditions is the output of the respective VCGEN.
