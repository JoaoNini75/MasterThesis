%!TEX root = ../template.tex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Config/7_package.tex
%% NOVA thesis configuration file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\typeout{NT FILE Config/5_packages.tex}%

% \ntlangsetup{pt/contentsname=O MEU ÍNDICE}
% \ntlangsetup{en/contentsname=MY LITTLE TOC}

%%============================================================
%%  FOR DEMO PURPOSES ONLY --- YOU SHOULD REMOVE THESE PACKAGES
%%============================================================
\usepackage{float}        % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{wrapfig}      % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\restylefloat{figure}     % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{metalogo}     % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\usepackage{lipsum}       % FOR DEMO PURPOSES ONLY --- PLEASE REMOVE
\IfFileExists{byo-twemojiss.sty}{
  \usepackage{byo-twemojis}     % to typeset emojis
  \newcommand{\emojiSmile}{\raisebox{-3pt}{\scalebox{1.3}{\byoTwemoji{head; eyes normal opened; mouth laughing}}}}
}{
  \newcommand{\emojiSmile}{\texttt{:D}}
  \newlength{\twemojiDefaultHeight}
}

\newcommand{\MikTeX}{Mik\TeX}
\newcommand{\TeXLive}{\TeX Live}
\newcommand{\pdfLaTeX}{pdf\LaTeX}
\definecolor{Schlgray}{gray}{0.95}


%%============================================================
%%  ADD YOUR PACKAGES AND OTHER CUSTOMIZATIONS HERE
%%============================================================

% \usepackage[orphans/prevent, widows/prevent]{widows-and-orphans} % add warnings about widows and orphans to “template.log”
% \usepackage{refcheck}     % list used and unused labels

\usepackage{paralist}     % To enable costumizable enumerates.  See the documentation.

\usepackage[textsize=footnotesize]{todonotes}  % To register TODO notes in the text

\usepackage{makecell}      % Multiline table cells and control vertical and horizontal alignment

\usepackage{tcolorbox}    % pretty print color boxes

% \usepackage{indentfirst}       % Indent first list after a (sub)section title

% \usepackage{algorithm2e}
% \RestyleAlgo{ruled}

% \usepackage{minted}    % Fontification of source code listings

\usepackage{listings}    % Fontification of source code listingsConfig/5_packages.tex
%                          % In alternative, use the package “minted” (highly recomended)

% \lstset{
%     captionpos=t,
%     basicstyle={\ttfamily\footnotesize},
%     numbers=left,
%     numberstyle={\ttfamily\tiny},
%     tabsize=2,
%     language=Java,
%     float,
%     frame=single,
%     columns=fullflexible,
%     breaklines=true,
%     postbreak=\mbox{\textcolor{red}{$\hookrightarrow$}\space},
%     inputencoding=utf8,
%     extendedchars=true,
%     literate=
%       {á}{{\'a}}1 {é}{{\'e}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {ú}{{\'u}}1
%       {Á}{{\'A}}1 {É}{{\'E}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {Ú}{{\'U}}1
%       {à}{{\`a}}1 {è}{{\`e}}1 {ì}{{\`i}}1 {ò}{{\`o}}1 {ù}{{\`u}}1
%       {À}{{\`A}}1 {È}{{\'E}}1 {Ì}{{\`I}}1 {Ò}{{\`O}}1 {Ù}{{\`U}}1
%       {ä}{{\"a}}1 {ë}{{\"e}}1 {ï}{{\"i}}1 {ö}{{\"o}}1 {ü}{{\"u}}1
%       {Ä}{{\"A}}1 {Ë}{{\"E}}1 {Ï}{{\"I}}1 {Ö}{{\"O}}1 {Ü}{{\"U}}1
%       {â}{{\^a}}1 {ê}{{\^e}}1 {î}{{\^i}}1 {ô}{{\^o}}1 {û}{{\^u}}1
%       {Â}{{\^A}}1 {Ê}{{\^E}}1 {Î}{{\^I}}1 {Ô}{{\^O}}1 {Û}{{\^U}}1
%       {œ}{{\oe}}1 {Œ}{{\OE}}1 {æ}{{\ae}}1 {Æ}{{\AE}}1 {ß}{{\ss}}1
%       {ç}{{\c c}}1 {Ç}{{\c C}}1 {ø}{{\o}}1 {å}{{\r a}}1 {Å}{{\r A}}1
%       {€}{{\EUR}}1 {£}{{\pounds}}1
% }

\usepackage{xspace}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{hyperref}
\usepackage[export]{adjustbox}
\usepackage{pgfgantt}
\usepackage{amssymb}
\usepackage{parcolumns}
\usepackage{placeins}

\newcommand{\whyml}{{WhyML}\xspace}
\newcommand{\whythree}{\gls{Why3}\xspace}
\newcommand{\cvc}{\gls{CVC4}\xspace}
\newcommand{\altergo}{\gls{Alt-Ergo}\xspace}
\newcommand{\zthree}{\gls{Z3}\xspace}
\newcommand{\ocaml}{{OCaml}\xspace}
\newcommand{\gospellang}{GOSPEL\xspace}
\newcommand{\cameleer}{\gls{Cameleer}\xspace}

\usepackage{textcomp}
\usepackage{graphicx}
\newsubfloat{figure}
\usepackage{xcolor}
\usepackage{colortbl}
\usepackage{rotating}
\usepackage{gospel}
\DeclareCaptionFormat{listing}{\hfill#3}
\captionsetup[lstlisting]{format=listing,singlelinecheck=false, margin=0pt,
  font={sf,it},labelsep=space,labelfont=bf,belowskip=-1pt}
  
\usepackage{mathpartir}

\lstdefinestyle{while_lang}{
  language=C,
  emptylines=1,
  breaklines=true,
  basicstyle=\ttfamily\color{black}\small,
  moredelim=**[is][\color{red}]{@}{@},
  moredelim=**[is][\color{blue}]{ç}{ç},
  keepspaces=true
}

\usepackage{bm}
\usepackage{amsmath}

\definecolor{myorange}{RGB}{220,100,0}

\usepackage{accents}
\usepackage{scalerel}   % \scalerel* to scale one symbol to another's width

% stretchy left-harpoon accent
\newcommand{\wideleftharpoonup}[1]{%
  \accentset{\scalerel*{\leftharpoonup}{\ensuremath{#1}}}{#1}%
}

% stretchy right-harpoon accent
\newcommand{\widerightharpoonup}[1]{%
  \accentset{\scalerel*{\rightharpoonup}{\ensuremath{#1}}}{#1}%
}

\usepackage{tabularx}          % <-- provides X column that wraps
\usepackage{calc}              % for \widthof etc if you want to tweak widths
\usepackage{array}

% helper for coloring keywords (works inside math mode)
\newcommand{\kw}[1]{\text{\ttfamily\bfseries\color{myorange}#1}}

% left column type (fixed proportion) and right column X (wraps)
\newcolumntype{L}{>{\raggedright\arraybackslash}p{0.45\linewidth}}
\newcolumntype{R}{>{\raggedright\arraybackslash}X}

